...
class Application extends \Phalcon\Mvc\Application {
	...
	protected function _registerServices() {
		...
		$di->set('mongo', function () use ($configs) {
				if (!$configs->mongo->username OR !$configs->mongo->password) {
					$mongo = new \MongoClient('mongodb://' . $configs->mongo->host.':'.$configs->mongo->port);
				} else {
					$mongo = new \MongoClient("mongodb://" . $configs->mongo->username . ":" . $configs->mongo->password . "@" . $configs->mongo->host.':'.$configs->mongo->port, array("db" => $configs->mongo->dbname));
				}
				return $mongo->selectDb($configs->mongo->dbname);
		}, TRUE);
		...
		$di->set('navigation', function() {
			return new Navigation($this->router);
		});
