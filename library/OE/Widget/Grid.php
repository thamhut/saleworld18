...
class Grid extends Base {
	...
	{ 
		...
		$htmlGridTemplate = '<div class="oe-grid-container grid-%s grid-style-%s" id="grid-%s">';
		
		$html  = sprintf($htmlGridTemplate, $this->name, $this->gridStyle, $this->name);
		$html .= sprintf('<form method="%s" action="%s" class="%s">', $this->method, $uri, $classForm);
	...
	protected function _getContent() {
		...
		if($this->gridStyle != 1 && !$this->disablePagination) {
